...
export const ThemeProvider: React.FC<ThemeProviderProps> = ({ children }) => {
  ...
  const fetchUserSchools = useCallback(async () => {
    ...
    nasaLog(
      "INFO",
      MODULE_TAG,
      "🎯 Starting comprehensive school fetch process",
      {
    ...
    try {
      if (!auth0User?.sub) {
        nasaLog(
          "WARN",
          MODULE_TAG,
          "⚠️ Skipping school fetch - no authenticated user",
          {
            userState: "null",
      ...
      nasaLog(
        "INFO",
        MODULE_TAG,
        "🎉 Successfully completed school fetch process",
        {
      ...
      nasaLog(
        "CRIT",
        MODULE_TAG,
        "💥 CRITICAL FAILURE in school fetching process",
        {
          errorMessage: err.message,
          errorType: err.constructor.name,
          errorStack: err.stack,
  ...
  useEffect(() => {
    if (auth0User?.sub) {
      nasaLog(
        "INFO",
        MODULE_TAG,
        "👤 User available, triggering school fetch",
        {
          auth0Id: auth0User.sub,
  ...
  const selectCurrentSchool = useCallback((schools: School[]) => {
    nasaLog(
      "INFO",
      MODULE_TAG,
      "🎯 Selecting current school from available options",
      {
    ...
    if (lastSchoolId) {
      ...
      if (selected) {
        ...
        nasaLog(
          "WARN",
          MODULE_TAG,
          "⚠️ School from localStorage not found in current schools",
          {
            lastSchoolId,
    ...
    if (!selected && schools.length > 0) {
      ...
      nasaLog(
        "INFO",
        MODULE_TAG,
        "🎯 Selected first available school as default",
        {
          schoolId: selected._id,
          schoolName: selected.schoolName,
  ...
  const theme = useMemo(() => {
    if (!currentSchool) {
      nasaLog(
        "DEBUG",
        MODULE_TAG,
        "🎨 Using default theme - no current school selected",
        {
