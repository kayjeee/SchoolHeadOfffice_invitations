...
import ReportsTab from '../../../../../components/parent/Dashboard/tabs/ReportsTab';
...
export default function ReportsPage(props: ParentDashboardPageProps) {
  const {
    school_slug,
    schoolName,
    email,
  ...
  if (!isAuthenticated) {
    return (
      <AuthGate
        ...
        returnTo={`/parent/${encodeURIComponent(school_slug)}/dashboard/${encodeURIComponent(email)}/reports`}
      />
    );
  }
  ...
  return (
    <ErrorBoundary>
      <Head>
        <title>{`${displayName} - Reports | Parent Portal`}</title>
      </Head>
      ...
      <FrontPageLayout user={initialProfile} userRoles={['parent']}>
        ...
        >
          <ReportsTab isPremium={isPremium} />
        </DashboardShell>
      </FrontPageLayout>
    </ErrorBoundary>
  );
