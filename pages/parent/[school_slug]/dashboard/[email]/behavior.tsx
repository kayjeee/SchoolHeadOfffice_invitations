...
import BehaviorTab from '../../../../../components/parent/Dashboard/tabs/BehaviorTab';
...
export default function BehaviorPage(props: ParentDashboardPageProps) {
  const {
    school_slug,
    schoolName,
    email,
  ...
  if (!isAuthenticated) {
    return (
      <AuthGate
        ...
        returnTo={`/parent/${encodeURIComponent(school_slug)}/dashboard/${encodeURIComponent(email)}/behavior`}
      />
    );
  }
  ...
  return (
    <ErrorBoundary>
      <Head>
        <title>{`${displayName} - Behavior | Parent Portal`}</title>
      </Head>
      ...
      <FrontPageLayout user={initialProfile} userRoles={['parent']}>
        ...
        >
          <BehaviorTab learners={initialLearners} isPremium={isPremium} />
        </DashboardShell>
      </FrontPageLayout>
    </ErrorBoundary>
  );
