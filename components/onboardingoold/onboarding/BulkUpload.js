...
const BulkUpload = ({
  ...
  // Debug: Log props on changes
  useEffect(() => {
    console.log('BulkUpload props:', { user, schools, selectedGrade });
  }, [user, schools, selectedGrade]);

  // Get auth0Id from multiple possible sources
  const getAuth0Id = () => {
    // Priority order: user prop, schools prop, fallback
    return user?.auth0_id || user?.sub || schools?.[0]?.userAuth0Id || '';
  };

  // Extract school and user info
  ...
  const validateAndParseFile = async file => {
    ...
    reader.onload = async e => {
      try {
        ...
        // Get school info and auth0Id
        const { schoolName, schoolEmail, userEmail, province, auth0Id } = getSchoolAndUserInfo();
        ...
        rows.forEach((row, i) => {
          ...
          const learner = {
            ...
            userEmail,
            province,
            auth0Id // Automatically attach auth0Id to each learner record
          };
  ...
  const handleConfirmUpload = async () => {
    ...
    // Get auth0Id and validate it's available
    const auth0Id = getAuth0Id();
    if (!auth0Id) {
      setErrorStatus('User authentication ID not found. Please try logging in again.');
      return;
    }
    ...
    try {
      // Automatically attach auth0Id in payload for onboarding updates
      const payload = {
        auth0Id: auth0Id, // Automatically get from props as requested
        data: validationResults.dataToUpload,
      };

      console.log('Upload payload with auth0Id:', { auth0Id, dataCount: payload.data.length });
      ...
      if (response.ok) {
        ...
        // Refetch onboarding status to ensure updates are reflected
        if (refetchOnboardingStatus) {
          try {
            await refetchOnboardingStatus();
            console.log('Onboarding status refreshed after upload');
          } catch (err) {
            ...
            {uploadStep === 'complete' && validationResults && (
              <div className="text-center py-10">
                ...
                <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="bg-green-50 rounded-lg p-3 text-center">
                    <div className="text-2xl font-bold text-green-600">{validationResults.inserted || 0}</div>
                    <div className="text-xs text-green-600">Learners Added</div>
                  </div>
                  <div className="bg-yellow-50 rounded-lg p-3 text-center">
                    <div className="text-2xl font-bold text-yellow-600">{validationResults.duplicates || 0}</div>
                    <div className="text-xs text-yellow-600">Duplicates Skipped</div>
                  </div>
                  <div className="bg-red-50 rounded-lg p-3 text-center">
                    <div className="text-2xl font-bold text-red-600">{validationResults.invalidRows || 0}</div>
                    <div className="text-xs text-red-600">Invalid Rows</div>
                  </div>
                </div>
