...
${schoolName} Admin Team`;
  ...
  async sendBulkMessages({
    ...
    // This is a simplified bulk sending logic. In a real application,
    // you would likely have a backend endpoint that handles this more robustly.
    const results = await Promise.all(
      recipientNumbers.map(async (to) => {
        try {
          const formattedNumber = to.replace(/\D/g, "");
          if (!formattedNumber.startsWith("27")) {
            throw new Error('Invalid phone number: must start with 27');
          ...
          const messageText = this.buildMagicLinkMessage(
            sanitizedSchoolName,
            magicLink,
            supportEmail
          );
